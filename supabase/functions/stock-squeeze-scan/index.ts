// Stock Squeeze Radar scanner. Assembles the stock universe (TTM-squeeze setups
// from daily candles) and scores it with the shared quant core using calibrated
// weights (falling back to priors).
import { createClient } from "npm:@supabase/supabase-js@2";
import { corsHeaders } from "npm:@supabase/supabase-js@2/cors";
import { scoreCrossSection, signalFor } from "../_shared/quant.ts";
import { buildStockItems, withWeights, STOCK_FACTORS } from "../_shared/scan.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const rows = await buildStockItems(supabase);
    if (!rows.length) {
      return new Response(JSON.stringify({ tokens: [] }), { headers: { ...corsHeaders, "Content-Type": "application/json" } });
    }

    const factors = await withWeights(supabase, "stock", STOCK_FACTORS);
